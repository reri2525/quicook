...
function Search(props) {
  ...
  useEffect(() => {
    setPostall([])
    window.scrollTo(0, 0);
    postSearchGet(props.searching);
  }, [currentPage, props.searching])
  const postShow = (e) => {
    setPostShowModal(true)
    setPostShowNumber(e)
  }
  const postSearchGet = (search) => {
     axios.get("http://localhost:3001/search", { params: { page: currentPage, search: search }, withCredentials: true})
     .then(response => {
